...
\newcommand{\vredok}[1]{\todo[inline,color=red!30]{\small\colorbox{green!30}{\textbf{Victor:}} #1}}
\newcommand{\rednote}[1]{\todo[color=red!30]{\small\textbf{Victor:} #1}}
\newcommand{\vblue}[1]{\todo[inline,color=blue!30]{\small\textbf{Victor:} #1}}
\newcommand{\vblueok}[1]{\todo[inline,color=blue!30]{\small\colorbox{green!30}{\textbf{Victor:}} #1}}
...
\bgroup
	...
	\begin{table}[htbp]
	  ...
	  \begin{tabular}{r c p{13cm}}
	    \toprule
            \(\boole{P}\) & \(\coloneqq\)& \(1\) se o predicado \(P\) for verdadeiro, e zero caso contrário \\
            \([n]\)
            & \(\coloneqq\)
...
\vgreen{Tava em dúvida se precisava de uma definição explícita, achei que provavelmente usaríamos isso só para deixar o texto mais limpo e que não faríamos contas com esses símbolos, mas parando pra pensar acho que vamos. Coloquei como uma \emph{notação} solta. Se acumular mais notações talvez valha a pena colocar tudo no começo numa seção de notação. Também to em dúvida sobre a numeração de teoremas/definições/notação: qual a lógica da sequência de números?}
\begin{equation}
  c(A, s) = \sum_{t=1}^{|s|} \boole{\sigma_t \notin A(s_{1:t-1})}
\end{equation}

Note que o primeiro da soma acima se reduz a \(\boole{\sigma_1 \notin [k]}\) pois \(s_{1:0} = \emptyseq\) e \(A(\emptyseq) = S_0 = [k]\) (veja \ref{seq_notation} e \ref{cache_ini}) .

\end{definition}



Ao longo do texto, por convenção, usaremos os símbolos "-" e "+" para denotar subtração e adição, respectivamente, de conjuntos ou de conjunto-elemento. (\textbf{\color{red}Victor}: Acabei de ver isso aqui. Isso merecia uma definição mais explicita e formal.)
...
\begin{definition}
  Seja \(\mathcal{A}\) o conjunto de algoritmos determinísticos para o problema de caching. Denotamos por \(\textbf{OPT}(\sigma)\) o custo mínimo para uma sequência de páginas \(\sigma\).
