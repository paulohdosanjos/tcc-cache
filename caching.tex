...
Os algoritmos de marcação são uma família de algoritmos de caching que seguem a seguinte estrutura. A execução do algoritmo é em fases, e no começo de cada fase, cada elemento do cache é dito \emph{desmarcado}. Quando um acerto de cache ocorre, o elemento correspondente é \emph{marcado}. Quando uma falha de cache acontece, alguma página desmarcada é descartada do cache, e o novo elemento é colocado no cache e imediatamente marcado. Se todas as páginas do cache estão marcadas e outra falha de cache acontece, então todo o cache é desmarcado e uma nova fase começa. Assim, um \textbf{algoritmo de marcação} é um algoritmo de caching que nunca descarta páginas marcadas, e seu funcionamente é resumido pela forma que escolhe elementos desmarcados para descartas após uma falha de cache.

\begin{definition}
  Um elemento \(z_i\) é chamado de \textbf{novo} para alguma fase \(r\) se apareceu na fase \(r\) mas não na fase \(r-1\). Se apareceu em ambas as fases, é chamado de \textbf{velho}. Uma \textbf{chegada} na fase \(r\) significa que o elemento \(z_i\) não apareceu anteriormente nesta mesma fase.
\end{definition}

\begin{lemma}
  \label{lem:opt_nova}
  Seja \(L\) o número de páginas novas de uma entrada finita \(\boldz \in \Seq(\Zcal)\). Então, \(L/2 \le \opt(\boldz) \le L\).
  \begin{proof}

    Defina as sequências \(\boldd\), \(\boldg\) e \(\boldh\) tais que \(g_i = |s_{j-1} - M_{j-1}|\), \(h_i = |s_{k+1}-M_{k}|\) e \(d_i = g_i - h_i\). Ou seja, ... Note que, \(g_{i+1} = h_i\). Toda página limpa requisitada nesta rodada que não estava em \(S_O\) força uma falha da OPT; assim, OPT tem ao menos \(L - d_I\) falhas. Além disso, \(d_F\) páginas presentes na OPT ao final não estavam no cache da \textsc{Marker} no início da rodada; para carregá-las, OPT teve ao menos \(d_F\) falhas. Portanto
\begin{equation*} 
  \opt(b) \ge \max\{L - d_i, d\} \ge \frac{L - d_I + d_F}{2}. 
\end{equation*} 

  Somando sobre todas os blocos rodadas, os termos \(-d_I\) e \(+d_F\) telescopam (entre final e início de rodadas consecutivas) e contribuem apenas \(O(1)\) no total, concluindo que 
\begin{equation*} 
  \opt(\boldz) \ge \frac12 \sum_r L O(1).
\end{equation*}
...
\begin{lemma}
  \label{lem:nova}
  Toda página nova incorre uma falha de cache.
  \begin{proof}
  \end{proof}
\end{lemma}
...
\begin{proof}
  Sejam \(\boldz \in \Seq(\Zcal)\) e \(b_j\) um elemento qualquer de \(\boldb(z)\), isto é, um bloco qualquer da entrada. Além disso, suponha que essa fase teve \(L\) páginas novas. Pelo \Cref{lem:nova}, estas \(L\) páginas incorrem \(L\) faltas.
